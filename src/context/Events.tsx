...
export const EventsProvider: FC = withContainer(({ children, container: { socket } }) => {
  ...
  const { eventsQueryString, statusFilter, paginationPage } = useSelector((state: ReduxState) => ({
  ...
  useEffect(() => {
    ...
    socket.on("updateEvent", async (updated: IEvent) => {
      console.log("updateEvent", updated.fromChain, updated.actionId, updated.status);
      ...
      try {
        ...
        setEvents([...events.slice(0, idx), { imgUri: "", ...updated }, ...events.slice(idx + 1)]); //updateEvent
      }
    });
  ...
  useEffect(() => {
    ...
    if (eventsQueryString.length && !statusFilter) {
      ...
      fetch(`${url}?chainName=` + eventsQueryString + `&sort=${sort}&offset=${paginationPage}`)
        .then((res) => res.json())
      ...
      fetch(`${url}?pendingSearch=` + eventsQueryString + `&sort=${sort}&offset=${paginationPage}`)
        .then((res) => res.json())
        .then(async ({ events, count }: { events: IEvent[]; count: number }) => {
          await loadImages(events, setEvents);
          setTotal(count);
