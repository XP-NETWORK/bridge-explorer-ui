        ...
        <div className="flex flex-col w-full">
            ...
            <div className="flex items-start justify-start gap-2 border-b py-4 detailsListRow">
                ...
                >
                    <a
                        href={`${
                            event?.toChain && txExplorers[event?.toChain]
                        }${event?.toHash && extractHash(event?.toHash)}`}
                        target="_blank"
                        rel="noreferrer"
                        className={`text-[#235EF5] ${
                            event?.toHash ? "" : "nonactive "
                        }`}
                    >
                        {truncate(
                            event?.toHash && extractHash(event?.toHash),
                ...
                >
                    <span className="text-[#235EF5]">
                        <a
                            ...
                            href={`${
                                event?.fromChain &&
                                addressExplorers[event?.fromChain]
                            }${encodeURIComponent(event?.senderAddress)}`}
                        >
                            {truncate(event?.senderAddress, truncateSize) ||
                                "N/A"}
                        </a>
                    </span>
                    {!dataLoad && (
            ...
            <div className="flex items-start justify-start gap-2 border-b py-4 detailsListRow">
                ...
                >
                    <a
                        ...
                        rel="noreferrer"
                        href={`${
                            event?.toChain && addressExplorers[event?.toChain]
                        }${encodeURIComponent(
                            event?.targetAddress ? event?.targetAddress : ""
                        )}`}
                    >
                        <span className="text-[#235EF5]">
                            {truncate(event?.targetAddress, truncateSize) ||
                                "N/A"}
                        </span>
                    </a>
                    {!dataLoad && (
