        ...
        events: [
            {
                ...
                toChainName: "BSC",
                fromHash:
                    "0x7fa21d343d83f76c97d838a1281b9763061589fae53ee1848fee0c88a687818b",
    ...
    const getParams = async () => {
        ...
        switch (true) {
            case searchParams.has("bar"):
                console.log("BOOOOOM", eventsContext);
                const search = searchParams.get("bar");
                const offset = searchParams.get("offset");
                setPageNumber(offset ? +offset : 0);
                const resp = await axios.get(
                    `${uri}?chainName=${search}&sort=DESC&offset=${offset}`
                );
                if (resp.data.events.length < 1) {
                    setEventsContext({
                        isLoading: false,
